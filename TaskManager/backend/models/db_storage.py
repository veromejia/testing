...
class DBStorage:
    ...
    def task_by_command(self,command):
        ...
        return query
        #task = self.__session.query(Task).filter(Task.task_command == command)
        #for task in self.__session.query(Task.command).filter(command):
    #def update_status_task(self,status):
